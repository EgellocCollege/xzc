...
class AlarmManagerService extends SystemService {
    ...
    private final class Constants extends ContentObserver {
        ...
        //　默认Alarm最小执行时间
        private static final long DEFAULT_MIN_FUTURITY = 5 * 1000;   // 5 秒
        // 默认Repeat Alarm的最短周期时间
        private static final long DEFAULT_MIN_INTERVAL = 60 * 1000;  // 1 分钟

        private static final long DEFAULT_MAX_INTERVAL = 365 * DateUtils.DAY_IN_MILLIS;
        // Doze IDLE下flag为ALLOW_WHILE_IDLE Alarm的最短时间间隔
        private static final long DEFAULT_ALLOW_WHILE_IDLE_SHORT_TIME = DEFAULT_MIN_FUTURITY;
        // Doze IDLE下flag为ALLOW_WHILE_IDLE Alarm的最长时间间隔
        ...
        public Constants(Handler handler) {
            ...
            // 更新Doze IDLE下flag为ALLOW_WHILE_IDLE Alarm的执行时间
    ...
    void calculateDeliveryPriorities(ArrayList<Alarm> alarms) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            // 计算 Alarm 的 priorityClass，为空的就计算
    ...
    void rebatchAllAlarmsLocked(boolean doValidate) {
        ...
        // 备份mAlarmBatches，mPendingIdleUntil
    ...
    public void onStart() {
        ...
        // mNextRtcWakeup - 下一个包含Rtc关机闹钟batch的start时间
        ...
        // 发布binderservice与localservice，分别供其他进程与system_server内部服务调用
    ...
    public void onBootPhase(int phase) {
        if (phase == PHASE_SYSTEM_SERVICES_READY) {
            ...
            // 获取AppOpsManager与DeviceIdleController服务对象
    ...
    private void setImplLocked(int type, long when, long whenElapsed, long windowLength,
        ...
        // set Alarm之前先移除前面设置的alarm
        removeLocked(operation, directReceiver);
        // 继续调用setImplLocked设置Alarm
    ...
    private void setImplLocked(Alarm a, boolean rebatching, boolean doValidate) {
        // FLAG_IDLE_UNTIL该定时器是Doze Idle定时器
    ...
    void rescheduleKernelAlarmsLocked() {
        ...
        if (mAlarmBatches.size() > 0) {
            ...
            // 更新mNextWakeup并调用native方法设置alarm
    ...
    //移除特定operation和directReceiver 的 alarm
    //  mAlarmBatches  mPendingWhileIdleAlarms  mPendingBackgroundAlarms mPendingIdleUntil
        ...
        while (mAlarmBatches.size() > 0) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if ((alarm.flags&AlarmManager.FLAG_ALLOW_WHILE_IDLE) != 0) {
                    // 如果时FLAG_ALLOW_WHILE_IDLE类型定时器，需要至少mAllowWhileIdleMinTime的时间间隔触发
                ...
                // 如果时mPendingIdleUntil触发，说明是Doze Idle状态时间到期
                ...
                if (alarm.wakeup) {
                    // 如果该alarm是wakeup类型，设置hasWakeup为true
                    hasWakeup = true;
                }
